...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if (empty($pesan_error)) {
        ...
        if (isset($stmt) && $stmt->execute()) {
            ...
            header("Location: kelola_dosen?status=" . $status_msg);
            exit;
        } else {
...
if (isset($_GET['hapus'])) {
    ...
    header("Location: kelola_dosen?status=hapus_sukses");
    exit;
}
                        ...
                        <?php $no = 1; foreach ($dosen_list as $i => $dosen): ?>
                            <tr>
                                ...
                                <td class="action-links">
                                    ...
                                    <a href="kelola_dosen?hapus=<?= $dosen['id'] ?>" class="btn-icon delete" onclick="return confirm('Yakin hapus?');" title="Hapus">
