...
if (isset($_GET['aksi']) && $_GET['aksi'] === "hapus" && isset($_GET['id'])) {
    ...
    try {
        ...
        if ($del->execute()) {
            ...
            header("Location: kelola_penelitian?status=hapus_sukses");
            exit;
        } else {
            header("Location: kelola_penelitian?status=hapus_gagal&msg=" . urlencode($del->error));
            exit;
        }
    } catch (Exception $e) {
        header("Location: kelola_penelitian?status=hapus_gagal&msg=" . urlencode($e->getMessage()));
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['mode_penelitian'])) {
    ...
    if (!$ok1 || !$ok2) {
        $err = !$ok1 ? $err1 : $err2;
        header("Location: kelola_penelitian?status=gagal_file&msg=" . urlencode($err));
        exit;
    }
    ...
    try {
        if ($mode === "tambah") {
            ...
            if ($sql->execute()) {
                header("Location: kelola_penelitian?status=tambah_sukses");
                exit;
            } else {
                header("Location: kelola_penelitian?status=tambah_gagal&msg=" . urlencode($sql->error));
                exit;
            }
            ...
            if ($sql->execute()) {
                header("Location: kelola_penelitian?status=edit_sukses");
                exit;
            } else {
                header("Location: kelola_penelitian?status=edit_gagal&msg=" . urlencode($sql->error));
                exit;
            }
        }

    } catch (Exception $e) {
        header("Location: kelola_penelitian?status=gagal&msg=" . urlencode($e->getMessage()));
        exit;
    }
}
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="action-links">
                                ...
                                <a href="kelola_penelitian?aksi=hapus&id=<?= $p['id'] ?>"
...
<div id="modalPenelitian" class="modal modal-xl modal-dialog-centered">
    <div class="modal-content">
        ...
        <form id="formPenelitian" action="kelola_penelitian" method="POST" enctype="multipart/form-data">
