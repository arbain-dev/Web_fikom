...
**a) Relasi Tabel `users` sebagai Pusat Autentikasi**

Tabel `users` merupakan entitas sentral yang menjadi titik kendali seluruh aktivitas administratif sistem Web FIKOM. Satu akun administrator (`users`) dapat mengelola banyak data `berita`, sehingga relasi antara kedua tabel ini bersifat *one-to-many*. Demikian pula dengan tabel `hero_slider`, di mana satu administrator dapat mengunggah dan mengaktifkan banyak gambar slide beranda. Berbeda dengan kedua tabel tersebut, relasi `users` dengan tabel `tentang_fikom` dan `halaman_statis` bersifat *one-to-one*, karena masing-masing tabel tersebut hanya memiliki satu baris data tunggal yang merepresentasikan konfigurasi konten profil dan halaman gambar statis yang dapat diperbarui kapan saja oleh administrator.

Tabel `visi_misi` dan `tb_fakta` juga berada dalam kendali langsung administrator dengan relasi *one-to-many*, mengingat setiap halaman visi, misi, tujuan, dan sasaran dapat memiliki banyak item teks, dan setiap fakta statistik yang ditampilkan di *homepage* dikelola sebagai baris data terpisah. Untuk domain kelembagaan, administrator memiliki wewenang penuh atas tabel `kerjasama` yang mencatat seluruh data instansi mitra, serta tabel `sop`, `rencana_strategis`, dan `rencana_operasional` yang berfungsi sebagai repositori dokumen resmi fakultas yang dapat diunggah dalam format PDF. Seluruh kegiatan kemahasiswaan yang tercatat dalam tabel `bem_struktur` dan `kalender_akademik` juga dikelola langsung oleh administrator melalui panel admin. Sementara itu, tabel `pendaftaran` yang berisi data calon mahasiswa baru bersifat *one-to-many* dari sisi administrator karena seorang admin dapat memverifikasi dan mengelola banyak data pendaftar yang masuk.

**b) Relasi Tabel `users` dengan `dosen` (Manajemen SDM)**

Tabel `dosen` berada di bawah yurisdiksi pengelolaan administrator dengan relasi *one-to-many*, di mana satu akun admin dapat menambahkan, memperbarui, dan menghapus banyak data dosen. Meskipun secara teknis tidak terdapat *foreign key* eksplisit pada level basis data, hubungan fungsional ini dijamin melalui mekanisme sesi PHP (`$_SESSION['admin_logged_in']`) yang memastikan hanya pengguna terautentikasi yang dapat memodifikasi data dosen. Dengan demikian, tabel `users` berperan sebagai *gate keeper* atas seluruh operasi yang berkaitan dengan data sumber daya manusia fakultas.

**c) Relasi Tabel `dosen` sebagai Pusat Domain Akademik**

Tabel `dosen` menjadi entitas induk bagi seluruh aktivitas Tri Dharma Perguruan Tinggi yang didokumentasikan dalam sistem. Satu dosen dapat terlibat dalam banyak kegiatan penelitian, sehingga tabel `dosen` memiliki relasi *one-to-many* terhadap tabel `penelitian`. Kolom `peneliti` pada tabel `penelitian` menyimpan nama dosen secara tekstual sebagai referensi fungsional. Hal serupa berlaku pada tabel `pengabdian`, di mana satu dosen dapat melaksanakan banyak kegiatan pengabdian kepada masyarakat sepanjang masa pengabdiannya. Relasi ini merepresentasikan kewajiban akademik dosen dalam memenuhi aspek penelitian dan pengabdian sebagaimana diatur dalam Tri Dharma Perguruan Tinggi.

Tabel `tabel_dosen` merupakan tabel referensi yang menyimpan salinan ringkas data dosen (nama, NIDN, dan program studi) untuk keperluan *dropdown* dan autofill pada formulir administratif. Tabel ini memiliki relasi fungsional *one-to-one* dengan masing-masing rekaman pada tabel `dosen`, berfungsi sebagai alias cepat tanpa perlu melakukan `JOIN` pada saat pengisian formulir. Adapun tabel `kurikulum`, `ruangan`, dan `laboratorium` dihubungkan dengan tabel `dosen` melalui relasi *one-to-many* yang bermakna: satu dosen dapat mengampu banyak kurikulum lintas semester, serta menggunakan berbagai ruangan kelas dan laboratorium sesuai jadwal akademik yang berlaku.

**d) Relasi Tabel `mahasiswa` dengan `pendaftaran`**

Tabel `mahasiswa` menyimpan data mahasiswa yang sudah resmi terdaftar dan aktif menjalani perkuliahan, sementara tabel `pendaftaran` merekam seluruh proses penerimaan mahasiswa baru (PMB) mulai dari pengisian formulir online hingga verifikasi berkas. Relasi *one-to-many* antara tabel `mahasiswa` dan `pendaftaran` bermakna bahwa satu rekaman mahasiswa dapat ditelusuri asal-usulnya dari satu atau lebih data pendaftaran yang pernah diajukan. Dalam praktik sistem ini, relasi tersebut lebih bersifat historis dan referensial: setelah calon mahasiswa diterima dan diregistrasi ke sistem, data mereka dipindahkan atau dibuatkan entri baru di tabel `mahasiswa`, sedangkan data di tabel `pendaftaran` tetap dipertahankan sebagai arsip proses seleksi.

**e) Ringkasan Jenis Relasi**

Secara keseluruhan, seluruh relasi antar tabel dalam basis data `db_web_fikom` diimplementasikan sebagai **relasi logis/fungsional** yang dijaga pada lapisan aplikasi PHP, bukan melalui *foreign key constraint* pada level basis data (*database-level referential integrity*). Pendekatan ini dipilih untuk memaksimalkan fleksibilitas dalam pengelolaan data dan memudahkan proses migrasi skema di masa mendatang. Terdapat dua jenis relasi utama yang digunakan: *one-to-many* (||--o{) untuk hubungan di mana satu entitas induk dapat memiliki banyak entitas anak (misalnya `users` dengan `berita`), dan *one-to-one* (||--||) untuk hubungan eksklusif di mana satu entitas hanya berpasangan dengan tepat satu entitas lain (misalnya `users` dengan `tentang_fikom`).
